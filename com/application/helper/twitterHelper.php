...
class twitterHelper {
	...
	function init(){
		global $TWITTER;
			...
			$this->tmhOAuth = new tmhOAuth(array(
							  'consumer_key'    => 'bZlpyF5wJp4Vu1O3A4DAj9BOG',
							  'consumer_secret' =>  'bmJsSUVNvD8JlqE4FM3Ql3K2oCJYxFGpLjzou4hkGmqD6wjkd9'
							));
			...
			// if(@isset($this->apps->session->getSession('twitter_session','twitter_permission')->loginPermission)){
				...
				if ($code == 200) {
					...
					if(!$this->apps->session->get('user')){
							if(isset($thisMobile))	sendRedirect($CONFIG['MOBILE_SITE']."home/loginTwiiter?loginType=twitter");
							else sendRedirect($CONFIG['BASE_DOMAIN']."home/loginTwiiter?loginType=twitter");
							//exit;
	...
	function request_login_link($flavourid=null,$module=null){
		...
		$modl=explode('/',$module);
		$userid = intval($this->apps->_g('userid'));
		if(isset($thisMobile))$callbackurl = "http://supersoccer.co.id/campusleague/home/loginTwiiter?loginType=twitter";
		else $callbackurl = "http://supersoccer.co.id/campusleague/home/loginTwiiter?loginType=twitter";
